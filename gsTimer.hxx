...
private:
	bool enabled;
	std::string timerID;
	std::deque<std::chrono::high_resolution_clock::time_point> times;
...
public:
	...
	int gsTimer::getElapsed() {
		 return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - times.front()).count();
	}
	void gsTimer::checkpoint(std::string message){

		times.push_front(std::chrono::high_resolution_clock::now());
		while (times.size() > 2)
			times.pop_back();
		if (enabled) {
			if (times.size() == 1) {
